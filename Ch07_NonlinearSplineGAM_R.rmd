...
model <- lm(Outstate ~ Private + Apps + Accept + Enroll, data = train_data)
...
gam_model <- gam(Outstate ~ s(Private, bs = "re") + s(Apps) + s(Accept) + s(Enroll), data = train_data)
...
rsquared <- cor(test_data$Outstate, test_predictions)^2
...
poly_model <- lm(mpg ~ poly(horsepower, 2), data = Auto)

spline_model <- gam(mpg ~ s(horsepower), data = Auto)

p1 <- ggplot(Auto, aes(x = horsepower, y = mpg)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  labs(title = "Linear Relationship (lm)")

p2 <- ggplot(Auto, aes(x = horsepower, y = mpg)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ poly(x, 2), se = FALSE, color = "red") +
  labs(title = "Quadratic Relationship (poly)")

p3 <- ggplot(Auto, aes(x = horsepower, y = mpg)) +
  geom_point() +
  geom_smooth(method = "gam", se = FALSE, color = "green") +
  labs(title = "Non-linear Relationship (gam)")
...
for (i in 1:1000) {
  ...
  beta0_estimates[i] <- beta0
  beta1_estimates[i] <- beta1
  beta2_estimates[i] <- beta2
}

plot(1:1000, beta0_estimates, type = "l", col = "red", xlab = "Iteration", ylab = "Estimate", main = "Estimates of beta0, beta1, and beta2")
lines(1:1000, beta1_estimates, col = "blue")
lines(1:1000, beta2_estimates, col = "green")
...
plot(1:1000, beta0_estimates, type = "l", col = "red", xlab = "Iteration", ylab = "Estimate", main = "Comparison of Estimates", ylim = range(c(beta0_estimates, coef(lm_model))))
lines(1:1000, beta1_estimates, col = "blue")
lines(1:1000, beta2_estimates, col = "green")

abline(h = coef(lm_model)["(Intercept)"], col = "red", lty = 2)
abline(h = coef(lm_model)["X1"], col = "blue", lty = 2)
abline(h = coef(lm_model)["X2"], col = "green", lty = 2)

legend("topright", legend = c("beta0", "beta1", "beta2", "lm_beta0", "lm_beta1", "lm_beta2"), 
...
Your code:
```{r,echo=TRUE}
mse_beta0 <- mean((beta0_estimates - coef(lm_model)["(Intercept)"])^2)
mse_beta1 <- mean((beta1_estimates - coef(lm_model)["X1"])^2)
mse_beta2 <- mean((beta2_estimates - coef(lm_model)["X2"])^2)

cat("MSE for beta0:", mse_beta0, "\n")
cat("MSE for beta1:", mse_beta1, "\n")
cat("MSE for beta2:", mse_beta2, "\n")
...
calculate_mse <- function(beta_hat, beta_true) {
  return(mean((beta_hat - beta_true)^2))
}

mse_values <- numeric(1000) 
...
for (iter in 1:1000) {
  ...
  mse_values[iter] <- calculate_mse(beta_hat, beta_true)
  
  if (iter > 1 && mse_values[iter] < 1e-6 * mse_values[iter - 1]) {
    break  
  }
}

plot(1:iter, mse_values[1:iter], type = "l", xlab = "Iteration", ylab = "Mean Squared Error", main = "MSE vs. Iteration")
